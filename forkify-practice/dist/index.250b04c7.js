...
var _stable = require("core-js/stable"); // polypiling everything else
var _runtime = require("regenerator-runtime/runtime"); // polypiling async/awawit
...
const controlRecipes = async function() {
    try {
        ...
        console.log(id);
        if (!id) return;
        _recipeViewJsDefault.default.renderSpinner(); // 1) Loading Recipe - loadRecipe() is async function so return a promise so that's why use 'await'
        // its not pure function so has a side-effect that manipulate state in model.
        await _modelJs.loadRecipe(id); // 2) Rendering recipe
...
},{"./model.js":"1hp6y","./views/recipeView.js":"9e6b9","core-js/stable":"1PFvP","regenerator-runtime/runtime":"62Qib","@parcel/transformer-js/src/esmodule-helpers.js":"367CR"}],"1hp6y":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
}; // fetching data from forkify API
const loadRecipe = async function(id) {
    try {
        const res = await fetch(`https://forkify-api.herokuapp.com/api/v2/recipes/${id}`);
        const data = await res.json();
        if (!res.ok) throw new Error(`${data.message} (${res.status})`); // rename the property of data in API
